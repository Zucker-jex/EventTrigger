...
- **需求物品仅作门槛**：需求物品**永不扣除**，仅检查玩家是否持有（`all` = 全部满足 / `any` = 任一满足）。
  真正扣款请配置**消耗物品**；配置界面不再提供"移除"勾选框。
...
5. **消耗物品模式**（`all` = 扣除全部消耗物品 / `any` = 任选其一扣除）
6. **冷却**
7. **奖励分支数量**（1–10）
8. **需求物品**（资格门槛，永不扣除，无勾选框）
9. **消耗物品**（可选）
10. **各分支奖励**
...
| `confirmDelivery`     | `{ id, batchCount, branchId, costOptionIndex, costIds }` | 确认交付            |
...
> 注：不再使用 `collect` 字段 —— 需求物品永不扣除（旧数据中残留的该字段会被忽略）。
